...
public static class Pico8Extensions
{
  ...
  public static Texture2D CircFill(GraphicsDevice graphicsDevice, float radius, Color color)
  {
    ...
    while (x >= y)
    {
      FillLine(data, textureSize, (int)(radius - x), (int)(radius + y), (int)(radius - x), (int)(radius - y), color);
      FillLine(data, textureSize, (int)(radius - y), (int)(radius + x), (int)(radius - y), (int)(radius - x), color);
      FillLine(data, textureSize, (int)(radius + x), (int)(radius + y), (int)(radius + x), (int)(radius - y), color);
      FillLine(data, textureSize, (int)(radius + y), (int)(radius + x), (int)(radius + y), (int)(radius - x), color);
  ...
  private static void FillLine(Color[] data, int textureSize, int x1, int y1, int x2, int y2, Color color)
  {
    if (y1 > y2)
    {
      int temp = y1;

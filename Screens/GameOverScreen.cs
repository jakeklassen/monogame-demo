...
{
	// Lose scene: a frozen snapshot of the last gameplay frame as a backdrop, the
	// game-over music, a blinking prompt, and any-input -> TitleScreen. The
	// highscore is persisted (new record highlighted) and shown. Source:
	// scenes/game-over-screen.ts.
	public class GameOverScreen(Game1 game) : GameScreenBase(game)
	{
		...
		public override void LoadContent()
		{
			...
			SoundSystem.Play(_world, "game-over");

			// "Game Over"
			var gameOverText = _world.Create();
			_world.Add(
				gameOverText,
				new Text()
				{

...
namespace Systems
{
	public class NextWaveEventSystem : SystemBase<GameTime>
	{
		...
		private readonly QueryDescription _eventEntities = new QueryDescription().WithAll<EventNextWave>();

		public NextWaveEventSystem(World world, Game1 game, Tweener tweener) : base(world)
		{
			_game = game;
			_tweener = tweener;
		}
		...
		{
			World.Query(in _eventEntities, (in Entity entity, ref EventNextWave nextWaveEvent) =>
			{
				// var nextWaveEvent = World.Get<EventNextWave>(entity);
				...
				for (var y = 0; y < wave.Enemies.Length; y++)
				{
					...
					for (var x = 0; x < row.Length; x++)
					{
						...
						Entity enemy = this.World.Create();
						World.Add(enemy, new BoxCollider(8, 8));
						World.Add(enemy, new CollisionLayer(CollisionMasks.Enemy));
						World.Add(enemy, new CollisionMask(CollisionMasks.Player | CollisionMasks.PlayerProjectile));
						World.Add(enemy, new EnemyState() { Value = EnemyStateType.Flyin });
						World.Add(enemy, new Invulnerable() { Duration = tweenDuration });
